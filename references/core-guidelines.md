...
4. 硬件约束优先于代码优化
5. 改代码之前先加观测点，没有观测手段的改动不提交
6. 一次只改一个逻辑点，每次改完 → 烧录 → 确认 → 提交
```
...
**详细流程见 [staged-verification.md](staged-verification.md)。**

```
阶段 1：外设单独验证（不写应用代码）
  → CubeMX 配置单个外设 → 编译 → 烧录 → 串口/万用表确认外设工作
  → 每个外设独立测试：UART → ADC → DAC → TIM → I2C → OLED
  → 验收标准：每个外设单独工作

阶段 2：数据链路验证（不接 UI）
  → TIM→ADC→DMA→串口打印 → 确认数据正确
  → DAC→ADC 自测 → 确认误差可接受
  → 验收标准：数据从 A 流到 B，值正确
...
**信息量**来自日志和验证，**改动量**来自拆分。两者同时做好，问题在引入时就会被发现。

### 1. 改代码之前先加观测点

- 改 I2C 逻辑之前，先加串口输出 I2C 返回值
- 改主循环之前，先加心跳日志
- 改中断之前，先加 GPIO 翻转用示波器看
- **没有观测手段的改动，不提交**

### 2. 一次只改一个逻辑点

- 想加整数格式化？先只加格式化函数，不动显示逻辑，测试通过再提交
- 想改 I2C 恢复？先只改恢复逻辑，不动显示函数，测试通过再提交
- 每次改完 → 烧录 → 确认 → 提交，循环控制在 5-10 分钟

### 3. 改之前先想清楚验证标准

- 改 I2C 恢复 → 故意拔掉排线再插上，能恢复显示
- 改整数格式化 → 串口输出格式正确
- 改动画逻辑 → 屏幕闪烁频率符合预期
- **没有验证标准的改动 = 盲改**
...
- I2C 失败 → 重新初始化 → 重试一次 → 再失败就放弃这次传输
- 不要加状态机、冷却期、计数器
- 恢复逻辑的代码量应该 ≤ 5 行
- **恢复逻辑的 bug 比原始 bug 更危险**（可能导致整个系统卡死）

### 5. 用 git bisect 代替手动回滚

```bash
git bisect start
git bisect bad          # 当前版本有问题
git bisect good <hash>  # 这个版本是好的
# 几轮之后 git 会告诉你 exactly 哪个 commit 引入了问题
```

### 6. Boot log 是嵌入式系统的标配

每个外设初始化后输出一行串口日志，出问题时看串口就知道卡在哪：
```c
...
// 使用
MX_USART1_UART_Init();
Boot_Log("[BOOT] UART1 OK");
MX_I2C1_Init();
Boot_Log("[BOOT] I2C1 OK");
OLED_Init();
Boot_Log("[BOOT] OLED OK");
```

### 7. 提交前自检清单

- [ ] 这个改动我能在 30 秒内说清楚它做了什么吗？
- [ ] 如果这个改动引入了 bug，我能快速定位吗？
- [ ] 我有没有加足够的日志来验证这个改动？
- [ ] 这个改动和上一个改动是独立的吗？

### 8. 碰到 HAL 问题先搜索

碰到 HAL 函数行为不符合预期、寄存器配置不生效、外设不工作时，**第一步是搜索，不是猜和试错。**

搜索关键词模板：
- `"HAL_<外设>_Start_DMA" <问题现象> site:community.st.com`
- `"STM32F4" <外设> <问题> workaround`
- `"STM32" HAL <函数名> bug`

参考应用笔记：AN3126（DMA）、AN4031（双缓冲）、UM1725（HAL 驱动文档）
