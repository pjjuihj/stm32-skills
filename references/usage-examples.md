...
// ⚠️ DMA 缓冲区必须是静态/全局变量，且对齐
static uint16_t adcBuffer[ADC_BUFFER_SIZE] __attribute__((aligned(4)));
static volatile bool adcReady = false;
static uint16_t startup_skip = 0;
...
void BSP_ADC_Start(void) {
    ...
    // ⚠️ 关键：HAL_ADC_Start_DMA 会覆盖 MspInit 的 DMA 配置
    // 必须手动修正 PSIZE/MSIZE 为 16bit，CIRC 为 1
...
// ⚠️ 因为直接操作了 DMA 寄存器，HAL 回调可能失效
// 需要在 DMA 中断中手动调用此回调
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc) {
    if (hadc->Instance == ADC1) {
        // 跳过启动后的前几帧（缓冲区可能未稳定）
        if (startup_skip < 20) { startup_skip++; return; }
        adcReady = true;
    }
}
...
### ❌ 错误 4：I2C 恢复逻辑过度设计
```c
// 错误！状态机导致 OLED 黑屏 5 天
static uint8_t i2c_bus_dead = 0;
static uint8_t i2c_cooldown = 0;
...
void OLED_WR_Byte(uint8_t dat) {
    ...
    if (HAL_I2C_Master_Transmit(...) != HAL_OK) {
        if (++err_count >= 3) i2c_bus_dead = 1;  // ← 永久禁用 I2C
    }
}
```
...
void OLED_WR_Byte(uint8_t dat) {
    ...
    if (HAL_I2C_Master_Transmit(&hi2c1, addr, data, 2, 100) == HAL_OK) return;

    // I2C bus recovery: 9 clock pulses + STOP
    I2C_Recovery();
    // 不重试，让下次调用再试
}
```
