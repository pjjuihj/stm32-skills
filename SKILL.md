...
发现问题
  │
  ├─→ ① 读寄存器确认状态（1min）→ 不猜，先看硬件实际状态
  │
  ├─→ ② 分层验证（硬件→驱动→数据→算法→显示）→ 逐层定位
  │
  ├─→ ③ 查项目文档（2min）→ 有答案 → 修复 → 验证 → 记录
  │
  ├─→ ④ 搜错误历史 → 看有没有踩过同样的坑
  │
  ├─→ ⑤ 搜网上资料（5min）→ ST Community / Stack Overflow
  │
  └─→ ⑥ 读 HAL 源码（5min）→ 最小改动修复 → 确认新代码 → 验证 → 记录
```

**为什么先读寄存器**：寄存器不会骗你。串口没输出？先看 USART->SR 的 TXE 位。DMA 不工作？先看 DMA->CR 的 EN 位。1 分钟确认硬件状态，比猜 10 分钟有效。
